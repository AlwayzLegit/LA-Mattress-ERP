...
describe('unexpected errors in the request log', () => {
  ...
  it('redacts chat session credentials with the current shared logger', async () => {
    lines.length = 0;
    await request(app.getHttpServer())
      .get('/boom/missing')
      .set('x-chat-session', 'private-chat-session-secret')
      .expect(404);
    expect(JSON.stringify(lines)).not.toContain('private-chat-session-secret');
