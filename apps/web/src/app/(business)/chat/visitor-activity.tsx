  ...
  return (
    <div className={styles.pageStrip} aria-label="Visitor’s latest page">
      ...
      {safePage ? (
        <>
          <a
            href={`https://www.mattressstoreslosangeles.com${page.path}`}
            target="_blank"
          ...
          <small>
            Last seen{' '}
            {new Date(page.seenAt).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}
          </small>
        </>
      ) : (
...
export function VisitorDraft({
  activity,
  assigned,
  typing,
}: {
  activity: ReturnType<typeof useVisitorActivity>;
  assigned: boolean;
  typing: boolean;
}) {
  ...
  return (
    <section
      className={styles.liveDraft}
      data-active={Boolean(draft)}
      aria-label="Visitor draft preview"
      data-sentry-mask
    >
      <div className={styles.draftHeading}>
        <strong>
          <span className={styles.draftDot} aria-hidden="true" />
          {draft ? 'Visitor is composing' : typing ? 'Visitor is typing' : 'Visitor draft preview'}
        </strong>
      ...
      {draft ? (
        ...
        <p className={styles.draftHint}>
          {!assigned
            ? 'Accept this chat to see drafts the visitor chooses to share.'
            : activity.unavailable
              ? 'Preview is temporarily unavailable. You can still reply.'
              : 'Opted-in drafts appear here while the visitor types.'}
        </p>
      )}
